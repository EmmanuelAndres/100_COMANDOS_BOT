...
module.exports = {
  ...
  run: async (bot, message, args, ops) => {
    ...
    if (!opponent) return message.channel.send("**Introduzca un usuario válido. Ex-duelquiz <@usuario> <puntos obligatorios 1-8>**");
    let maxPts = args[1];
    if (maxPts <= 0 || maxPts >= 9) return message.channel.send("**Introduzca un número entre 1 y 8.**");
    if (opponent.user.bot) return message.channel.send('**¡No se puede jugar contra bots!**');
