...
module.exports = {
    ...
    run: async (bot, message, args, ops) => {
        ...
        if (opponent.user.bot) return message.channel.send('**¡No se puede jugar contra los bots!**');
        ...
        try {
            ...
            while (!winner && board.some(row => row.includes(null))) {
                ...
                await message.channel.send(stripIndents`
					**${user}, ¿Qué columna desea elegir? Escriba \`end\` ¡A renunciar**
					${displayBoard(board)}
					${nums.join('')}
				`);
                const filter = res => {
                ...
                if (choice.toLowerCase() === 'end') {
                    ...
                    await message.channel.send(`**${winner} gano!**`)
                    ops.games.delete(message.channel.id)
                }
                const i = Number.parseInt(choice, 10) - 1;
            ...
            return message.channel.send(winner ? `**Felicitaciones, ${winner}!**` : '**¡Es un empate!**');
        } catch (err) {
            ops.games.delete(message.channel.id);
            throw err;
        }
